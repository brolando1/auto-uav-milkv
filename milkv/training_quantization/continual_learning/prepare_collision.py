...
try:
    ...
    from .compat import encoder_fingerprint
    from .runtime_utils import load_dumped_latents
except Exception:
    from lr_data import list_pairs_collision
    from setup_from_config import DataConfig
    from samples import CollisionSample
...
def prepare_collisions(data_config: DataConfig) -> None:
    ...
    # Fast path: the Duo S node dumps the latents it computed at inference time
    # (latents.npy + latents_meta.json next to camera_images/). They are only
    # used if tap, sample count and the fingerprint of the pre-tap weights match.
    buf = None
    try:
        fp = encoder_fingerprint(data_config.original_model, data_config.latent_tap)
        Z_np, why = load_dumped_latents(nog_root, tap=data_config.latent_tap, encoder_fp=fp, expected_n=len(samples))
    except Exception as e:  # never let the fast path break a finetune
        Z_np, why = None, f"error ({e})"
    if Z_np is not None:
        import torch
        Z_all_t = torch.from_numpy(Z_np).to(data_config.device, dtype=torch.float32)
        buf = LatentBuffer(capacity=int(Z_all_t.shape[0]))
        buf.tap = data_config.latent_tap
        buf.Z_all_t = Z_all_t
        buf.Y_all_t = torch.zeros((int(Z_all_t.shape[0]), 1), dtype=torch.float32, device=data_config.device)
        print(f"[prepare_collision] using {int(Z_all_t.shape[0])} latents dumped by the node (no encoder pass)")
    else:
        print(f"[prepare_collision] computing latents with the encoder: {why}")
