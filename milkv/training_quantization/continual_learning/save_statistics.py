...
try:
    ...
    from .compat import export_classifier_onnx as _export_classifier_onnx
...
def save_statistics(data_config: DataConfig) -> None:
    ...
    if bool(getattr(data_config, "save_checkpoint", True)):
        ...
        # ONNX copy next to it (probability + latent) for onnxruntime inference on the node
        try:
            import time as _time
            _t0 = _time.perf_counter()
            onnx_out = _export_classifier_onnx(model, ckpt_out.with_suffix(".onnx"), str(data_config.latent_tap))
            print(f"[CHECKPOINT] ONNX export: {onnx_out} ({_time.perf_counter() - _t0:.1f}s)")
        except Exception as e:
            print(f"[CHECKPOINT] WARNING: ONNX export failed, the node will run this checkpoint with torch: {e}")
