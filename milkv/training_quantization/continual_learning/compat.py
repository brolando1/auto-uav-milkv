...
# =============================================================================
# Latent reuse between the node's inference and the finetune
# =============================================================================
def is_post_tap_param(name: str, tap: str) -> bool:
    """True for parameters that latents_only training updates for this tap
    (same rules as setup_from_config._prepare_model_for_latents_only)."""
    if tap == "post_merge":
        return name.startswith(("combined_block_1", "combined_block_2", "fully_connected"))
    if tap == "post_comb1":
        return name.startswith(("combined_block_2", "fully_connected"))
    if tap in ("post_comb2", "pre_fc"):
        return name.startswith("fully_connected")
    raise ValueError(f"Unsupported tap: {tap}")


def encoder_fingerprint(model, tap: str) -> str:
    """SHA1 over every parameter and buffer *before* the tap, i.e. over the part
    of the network that produces the latent. Two models with the same
    fingerprint produce identical latents at this tap, so latents computed by
    the node at inference time can be reused by the finetune."""
    import hashlib
    h = hashlib.sha1()
    h.update(tap.encode())
    for name, t in sorted(model.state_dict().items()):
        if is_post_tap_param(name, tap):
            continue
...
@torch.no_grad()
def infer_with_latent(model, img, tof, tap: str):
    """Full forward split at the tap: returns (probability [B,1], latent Z).
    Numerically the same as model(img, tof), see tests in duos_node/tests."""
    z = extract_latent(model, img, tof, tap=tap)
    return forward_from_latent(model, z, tap=tap), z


# =============================================================================
# ONNX export of the classifier (probability + latent), for onnxruntime on the
# Duo S: ~3.5x faster than torch's generic CPU kernels there, same numbers.
# =============================================================================
class ClassifierWithLatent(torch.nn.Module):
    """model(img, tof) split at `tap`: returns (p_gate [B,1], latent Z)."""

    def __init__(self, model, tap: str):
        super().__init__()
        self.m = model
        self.tap = tap

    def forward(self, img, tof):
        z = extract_latent(self.m, img, tof, tap=self.tap)
        return forward_from_latent(self.m, z, tap=self.tap), z


def export_classifier_onnx(model, onnx_path, tap: str, opset: int = 17):
    """Writes onnx_path (atomically) with inputs image[1,1,168,168], tof[1,1,21,21]
    and outputs p_gate[1,1], latent. Needs the `onnx` package (python3-onnx)."""
    import os
    import torch.onnx
    ...
    model.eval()
    try:
        wrapper = ClassifierWithLatent(model, tap).eval()
        img = torch.zeros(1, 1, 168, 168, dtype=torch.float32)
        tof = torch.zeros(1, 1, 21, 21, dtype=torch.float32)
        with torch.no_grad():
            torch.onnx.export(
                wrapper, (img, tof), tmp,
                input_names=["image", "tof"], output_names=["p_gate", "latent"],
...
def warm_onnx_exporter(model_cls, tap: str = "post_comb1") -> None:
    """Run one throwaway export so torch.onnx's lazy imports are loaded (a
    forked training child then exports in ~3 s instead of ~6 s on the Duo S)."""
    import os
    import tempfile
    m = model_cls().eval()
    ...
    os.close(fd)
    try:
        export_classifier_onnx(m, path, tap)
    finally:
        try:
            os.unlink(path)
        except FileNotFoundError:
