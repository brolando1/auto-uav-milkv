...
#define CAM_WIDTH 168
#define CAM_HEIGHT 168

static pi_task_t task1;
//declare buffer array for double buffering
static unsigned char *imgBuff[2];
static struct pi_device camera;
static pi_buffer_t buffer[2];
...
void camera_task(void *parameters)
{
  ...
  imgBuff[0] = (unsigned char *)pmsis_l2_malloc(captureSize);
  imgBuff[1] = (unsigned char *)pmsis_l2_malloc(captureSize);

  if (imgBuff[0] == NULL||imgBuff[1] == NULL)
  {
  ...
  pi_buffer_init(&buffer[0], PI_BUFFER_TYPE_L2, imgBuff[0]);
  pi_buffer_set_format(&buffer[0], CAM_WIDTH, CAM_HEIGHT, 1, PI_BUFFER_FORMAT_GRAY);
  pi_buffer_init(&buffer[1], PI_BUFFER_TYPE_L2, imgBuff[1]);
  pi_buffer_set_format(&buffer[1], CAM_WIDTH, CAM_HEIGHT, 1, PI_BUFFER_FORMAT_GRAY);
  ...
  uint32_t imgSize = 0;
  int first_frame = 1;
  int cam_buffer_idx = 0;
  int enc_buffer_idx = 1;
  int frame_counter = 0;
  ...
  while (1)
  {
    if (wifiClientConnected == 1)
    {
      ...
      if(first_frame == 1){ //camera can have issues if started before while loop
        first_frame = 0;
        pi_camera_control(&camera, PI_CAMERA_CMD_START, 0);
        pi_camera_capture_async(&camera, imgBuff[cam_buffer_idx], resolution, pi_task_callback(&task1, capture_done_cb, NULL));
      }
      xEventGroupWaitBits(evGroup, CAPTURE_DONE_BIT, pdTRUE, pdFALSE, (TickType_t)portMAX_DELAY);

      //swap buffers
      int temp = cam_buffer_idx;
      cam_buffer_idx = enc_buffer_idx;
      enc_buffer_idx = temp;

      //restart capturing
      pi_camera_capture_async(&camera, imgBuff[cam_buffer_idx], resolution, pi_task_callback(&task1, capture_done_cb, NULL));
      //only send every second frame:
      // frame_counter++;
      ...
      if (streamerMode == JPEG_ENCODING)
      {
        ...
        start = xTaskGetTickCount();
        //encode from correct buffer:
        jpeg_encoder_process(&jpeg_encoder, &buffer[enc_buffer_idx], &jpeg_data, &jpegSize);
